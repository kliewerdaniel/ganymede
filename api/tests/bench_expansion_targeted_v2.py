"""Targeted query expansion v2: inline replacement for legal terms + date normalization.

Design: Apply expansions ONLY for terms actually present in the question.
- Date normalization: replace "date" with "DATE date" (for date questions only)
- Legal-term synonyms: inline replacement (breach→default breach, notice→notice demand, etc.)
- Do NOT stack all expansions — only apply what's relevant to the question.

This is NOT the rejected "combined" variant which applied ALL expansions to ALL queries.
"""
...
# Inline legal-term synonym mappings (only applied if the term is present)
LEGAL_SYNONYMS_INLINE = [
    ...
    ("contract price", "amount total balance"),  # for Q09
    ("received notice", "served with notice"),  # for Q25
    ...
    ("cure period", "cure period grace period"),
...
def targeted_expand_v2(question: str) -> str:
    """Apply targeted inline expansion based on question content.

    1. Date normalization: replace "date" with "DATE date" for date questions.
    2. Legal-term synonyms: inline replacement for terms actually present.
    3. Do NOT apply all expansions — only those relevant to the question.
    """
    q = question
    ...
    # Legal-term synonyms (inline replacement, only for terms present)
    for term, replacement in LEGAL_SYNONYMS_INLINE:
        if term in q.lower():
...
def main():
    ...
    try:
        ...
        print("=" * 70)
        print("Targeted Query Expansion v2 — 6 Structural Misses")
        print("=" * 70)
        ...
        for q_id, question, expected_doc in MISSES:
            ...
            # Targeted expansion v2
            expanded_q = targeted_expand_v2(question)
            t0 = time.time()
            ...
            print(f"  {q_id}: original={status_orig} targeted_v2={status_exp}")
            print(f"    Original:    {question[:80]}")
            print(f"    Targeted v2: {expanded_q[:80]}")
        ...
        exp_hits = sum(1 for r in results if r["targeted_v2"]["found"])
        print(f"  Original:    {orig_hits}/6 hits")
        print(f"  Targeted v2: {exp_hits}/6 hits")
        print(f"  Delta:       {exp_hits - orig_hits:+d}")

        recovered = [r["q_id"] for r in results if r["targeted_v2"]["found"] and not r["original"]["found"]]
        lost = [r["q_id"] for r in results if not r["targeted_v2"]["found"] and r["original"]["found"]]
        if recovered:
        ...
        report = {
            ...
            "variant": "targeted_date_legal_v2_inline",
        ...
        report_path = os.path.join(report_dir, "expansion-targeted-v2-report.json")
