#!/usr/bin/env python3
"""Threshold sweep for NLI verifier on gold set.

Phase 1: Collect NLI entailment scores for every (question, citation) pair.
...
def collect_scores():
    """Phase 1: collect NLI scores for every question."""
    db = SessionLocal()
    ...
    output_path = os.path.join(os.path.dirname(__file__), "nli-scores.json")
...
def sweep(data):
    ...
    output_path = os.path.join(os.path.dirname(__file__), "nli-threshold-sweep-results.json")
    with open(output_path, "w") as f:
...
if __name__ == "__main__":
    scores_path = os.path.join(os.path.dirname(__file__), "nli-scores.json")
